...
const todoNextStepDate = (days: number) => {
                            ...
                            {(() => {
                              ...
                              return (
                                <div style={{ lineHeight: 1.5 }}>
                                  ...
                                  <div style={{ fontSize: 12, color: '#64748b', marginTop: 1 }}>{todoNextStepDate(info.days)}</div>
                                </div>
                              );
                            })()}
                          </td>
            ...
            {!done ? (
              <>
                ...
                {(() => {
                  ...
                  const currentPhaseActions = PHASE_ACTIONS[currentPhase] ?? [];
                  const effectivePhaseAction = currentPhaseActions.length === 1 ? (phaseAction || currentPhaseActions[0]) : phaseAction;
                  ...
                  const isSchedulingAction = /schedul/i.test(effectivePhaseAction);
                  const isNoAnswerAction = currentPhase === 'contact' && effectivePhaseAction === 'Re-attempted contact — no answer';
                    ...
                    <>
                      ...
                      <div style={{ marginBottom: 12 }}>
                        ...
                        <select
                          ...
                          style={{ width: '100%', boxSizing: 'border-box', border: '1.5px solid #e2e8f0', borderRadius: 8, padding: '7px 10px', fontSize: 13, color: effectivePhaseAction ? '#1e293b' : '#94a3b8', outline: 'none', fontFamily: 'inherit', background: '#fff', appearance: 'none', cursor: 'pointer' }}
                      ...
                      {(!!effectivePhaseAction || !!workflowOutcome) && <>
                      ...
                      {isNoAnswerAction && (
                            ...
                            {currentPhase === 'contact' && effectivePhaseAction === 'Called client — walked through portal together' ? (
                              <button className="ccbhc-primary-btn" style={{ width: '100%' }} onClick={() => {
                                ...
                                const entry = { phase: completedPhase.id, label: completedPhase.label, action: effectivePhaseAction, note: '', date: TODAY, actor: assignedTo || assignedNavigator || undefined };
                            ...
                            {!isSchedulingAction && <div ref={outcomeRef} style={{ marginBottom: 12 }}>
                              ...
                              {workflowOutcome === 'escalate' && (
                                <div>
                                  <select value={outcomeReason} onChange={e => { const r = e.target.value; setOutcomeReason(r); persist({ outcomeReason: r, pingDate: '' }); onStatusChange?.(clientId, 'Escalated to supervisor'); if (r) { const completedPhase = PHASES[phaseIndex]; const actionEntry = { phase: completedPhase.id, label: completedPhase.label, action: phaseAction, note, date: TODAY, actor: assignedTo || assignedNavigator || undefined }; const outcomeEntry = { phase: 'outcome', label: 'Outcome', action: `Escalate — ${r}`, note: '', date: TODAY, actor: assignedTo || assignedNavigator || undefined }; const newHistory = [outcomeEntry, actionEntry, ...phaseHistory.filter(h => h.phase !== 'outcome' || !h.action.startsWith('Escalate'))]; setPhaseHistory(newHistory); persist({ outcomeReason: r, pingDate: '', phaseHistory: newHistory }); } }} style={{ width: '100%', boxSizing: 'border-box', border: '1.5px solid #fde68a', borderRadius: 8, padding: '7px 10px', fontSize: 13, color: outcomeReason ? '#1e293b' : '#94a3b8', outline: 'none', fontFamily: 'inherit', background: '#fffbeb', appearance: 'none', cursor: 'pointer' }} onFocus={e => { e.target.style.borderColor = '#b45309'; e.target.style.boxShadow = '0 0 0 3px #fef3c7'; }} onBlur={e => { e.target.style.borderColor = '#fde68a'; e.target.style.boxShadow = 'none'; }}>
                                    <option value="">Select reason…</option>
                            ...
                            {workflowOutcome === 'escalate' && outcomeReason ? (
                              <div style={{ display: 'flex', gap: 8 }}>
                                  <button className="ccbhc-primary-btn" style={{ flex: 1, background: '#b45309', fontSize: 13 }} onClick={() => {
                                    const completedPhase = PHASES[phaseIndex];
                                    const actionEntry = { phase: completedPhase.id, label: completedPhase.label, action: effectivePhaseAction, note, date: TODAY, actor: assignedTo || assignedNavigator || undefined };
